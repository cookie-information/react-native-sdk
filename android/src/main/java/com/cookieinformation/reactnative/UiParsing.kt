...
object UiParsing {
    ...
    fun buildColorSchemeFromReadableMap(map: ReadableMap?, isDark: Boolean): ColorScheme? {
        if (map == null) return null
        val primary = if (map.hasKey("primary")) parseColorCode(map.getString("primary"))?.let { Color(it) } else null
        val secondary = if (map.hasKey("secondary")) parseColorCode(map.getString("secondary"))?.let { Color(it) } else null
        val tertiary = if (map.hasKey("tertiary")) parseColorCode(map.getString("tertiary"))?.let { Color(it) } else null
        if (primary == null && secondary == null && tertiary == null) {
            return null
        }
        return if (isDark) {
    ...
    fun buildTypographyFromReadableMap(map: ReadableMap?, activity: ComponentActivity): Typography? {
        if (map == null) return null
        val bodyMediumMap = if (map.hasKey("bodyMedium")) map.getMap("bodyMedium") else null
        val bodyMedium = parseTextStyleFromReadableMap(bodyMediumMap, activity)
        if (bodyMedium == null) return null
        return Typography().copy(bodyMedium = bodyMedium)
    }
    ...
    private fun parseTextStyleFromReadableMap(map: ReadableMap?, activity: ComponentActivity): TextStyle? {
        if (map == null) return null
        val size = if (map.hasKey("size")) map.getInt("size") else null
        val fontName = if (map.hasKey("font")) map.getString("font") else null
        val fontFamily = fontName?.let {
            val resId = activity.resources.getIdentifier(it, "font", activity.packageName)
            if (resId == 0) null else FontFamily(Font(resId))
        }
